 ...
 * }
 */
export async function embedBookHandler(
  bookId: string,
  text: string,
  title: string,
  author: string,
 ...
 * }
 */
export async function similarityHandler(textA: string, textB: string) {
  try {
 ...
 * }
 */
export async function compareEmbeddingsHandler(
  embeddingA: number[],
  embeddingB: number[],
) {
  try {
    ...
    if (embeddingA.length !== 384 || embeddingB.length !== 384) {
      throw new Error(
        `Embeddings must be 384-dimensional (got ${embeddingA.length} and ${embeddingB.length})`,
      );
    }

...
import { analyzePage, getTopWords, resetFrequency, } from "../analytics/wordFrequency";
import { Book } from "../db/book.model";
export async function analyzeBookFromUpload(file, title, author, topN, onProgress) {
    // Check if book already exists (deduplication)
    const existingBook = await Book.findOne({ title, author });
    if (existingBook && existingBook.topWords?.length) {
        console.log(`📚 Book "${title}" by ${author} already analyzed. Returning cached results.`);
        return existingBook;
    }
    ...
    const book = await Book.findOneAndUpdate({ title, author }, {
        title,
        author,
        s3Key: key,
        topWords: result,
    ...
    try {
        ...
        embeddings = await embedBookAutomatically(book._id.toString(), fullText, title, author);
    }
    catch (error) {
        console.error(`⚠️ Failed to automatically embed book "${title}":`, error.message);
...
async function embedBookAutomatically(bookId, text, title, author) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`ML Service returned ${response.status}: ${errorText || response.statusText}`);
        }
        const result = await response.json();
        console.log("json embedded result:", result);
        console.log(`✅ Book "${title}" (ID: ${bookId}) automatically indexed in ML service`);
        return result;
    }
    catch (error) {
        console.error(`❌ ML embedding error for "${title}":`, error.message);
        throw error;
    }
}
export async function listBooks(page = 1, limit = 10) {

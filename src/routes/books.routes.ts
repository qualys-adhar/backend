  ...
  try {
    ...
    if (!book.embedding || book.embedding.length === 0) {
      return c.json({
        ...
        message:
          "Book does not have an embedding yet. Similarity comparison unavailable.",
      });
    }
    ...
    const similarityPromises = otherBooks.map(async (otherBook) => {
      try {
        const result = await mlClient.compareEmbeddings(
          book.embedding!,
          otherBook.embedding!,
        );
        return {
...
router.post("/similarity", async (c) => {
  try {
    ...
    if (!book_id_1 || !book_id_2) {
      return c.json(
        { error: "Both book_id_1 and book_id_2 are required" },
        400,
      );
    }
    ...
    console.log(
      `[Backend] Comparing books: "${book1.title}" vs "${book2.title}"`,
    );

    // Call ML service to compute similarity using pre-computed embeddings
    const similarityResult = await mlClient.compareEmbeddings(
      book1.embedding,
      book2.embedding,
    );

    console.log(
      `[Backend] Similarity score: ${similarityResult.similarity.toFixed(4)}`,
    );
